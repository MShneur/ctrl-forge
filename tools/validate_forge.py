#!/usr/bin/env python3
"""Validate a CTRL-FORGE repo. Stdlib only."""
...
REQUIRED_FILES = [
    ...
    "tools/public_boundary_check.py",
]
...
def main() -> int:
    ...
    boundary = subprocess.run(
        [sys.executable, str(repo / "tools" / "public_boundary_check.py")],
        cwd=repo,
        text=True,
        capture_output=True,
        check=False,
    ...
    if boundary.returncode != 0:
        errors.append(boundary.stderr.strip() or boundary.stdout.strip() or "Public boundary check failed")
